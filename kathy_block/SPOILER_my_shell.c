...
int bootstrapDeviceFromMasterBlock(block_device_t bd, master_block_t mb){
    readMasterBlock(bd, mb, sizeof(MasterBlock));
    return 0;
}
    ...
    while (!done && (buf = readline(">> ")) != NULL) {
        ...
        if (prefix("quit", buf)) {
            ...
            if (num_words == 4) {
                ...
                BitSet_p bs = allocBlockMap(block_count);
            ...
            if (num_words == 2 && device_name != NULL) {
                ...
                main_mb = allocMasterBlock(sizeof(MasterBlock),0,0);
                if (bootstrapDeviceFromMasterBlock(main_bd, main_mb) > 0) {
                    // now main_bd points to a working, initialized block device
                    // allocate a blank Master Block
                    // now fill it in
                    main_bs = allocBlockMap(main_bd->m_blockCount);
                    if (readMasterBlock(main_bd, main_mb, 0) == 0 &&
                        ((readBlockMap(main_bd, main_bs, 1)) != 0)) {
            ...
            if (main_bd != NULL) {
                ...
                // close the file
                int closeZero = closeDevice(main_bd);
                // set the mb to NULL
                main_bd = NULL;
                main_mb = NULL;
